...
$req=$bdd->prepare('SELECT p.pa_dossier dossier,p.pa_numero numero,sr.sr_numero sr_numero,DAY(sr.sr_dateprelevement) AS jour,MONTH(sr.sr_dateprelevement) AS mois,
YEAR(sr.sr_dateprelevement) AS annees FROM patient p INNER JOIN serologie sr on p.pa_numero=sr.sr_dossier ORDER BY numero DESC LIMIT :nbre_page,:page');
