...
$req=$bdd->prepare('SELECT p.pa_dossier dossier,p.pa_numero numero,cv.cv_numero cv_numero,DAY(cv.cv_dateprelevement) AS jour,MONTH(cv.cv_dateprelevement) AS mois,YEAR(cv.cv_dateprelevement) AS annees,
DAY(cv.cv_daterendue) AS jour_r,MONTH(cv.cv_daterendue) AS mois_r,YEAR(cv.cv_daterendue) AS annees_r
FROM patient p INNER JOIN chargevirale cv on p.pa_numero=cv.cv_dossier ORDER BY cv.cv_daterendue DESC LIMIT :nbre_page,:page');
...
?>
	...
	<tr class="success">
		...
		<td><a target="_blank" href="page_cv_impression.php?page=<?php echo $donnees['cv_numero'];?>">Imprimer</a></td>
	</tr>
